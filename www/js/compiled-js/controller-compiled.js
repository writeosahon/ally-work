...
$('#hour-glass-loader-modal .modal-message').html("Authorizing Wallet Fund...");return Promise.all([].concat(_toConsumableArray(responseArray),[$('#hour-glass-loader-modal').get(0).show()]));}).then(function(responseArray){// submit the form data
...
processData:true,data:{raverefid:responseArray[0].data.flwRef,otp:responseArray[1],phone:utopiasoftware.ally.model.appUserDetails.phone}// data to submit to server
...
}).then(function(userDetails){// save the user details to encrypted storage
return utopiasoftware.ally.saveUserAppDetails(userDetails);}).then(function(){return $('#hour-glass-loader-modal').get(0).hide();// hide loader
}).then(function(){return Promise.all([ons.notification.toast("Wallet Transfer Successful!",{timeout:4000}),$('#app-main-navigator').get(0).popPage({data:{refresh:true}})]);}).catch(function(err){if(typeof err!=="string"){// if err is NOT a String
...
contactPhoneNumber=contactPhoneNumber.replace("0","+234");}// update the recipient phone input field with the retrieved & formatted phone number
