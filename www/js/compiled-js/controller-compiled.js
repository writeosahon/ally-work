...
new Promise(function(resolve,reject){ons.notification.confirm('To complete sign up, your phone number must be verified. <br>'+'You will receive an SMS with your verification code shortly.<br> '+"Tap 'OK' to proceed",{title:'Verify Phone Number',buttonLabels:['Cancel','Ok']})// Ask for confirmation
.then(function(index){if(index===1){// OK button
...
$(buttonElement).attr("data-ally-visible","no");// flag the pin is now invisible
         ...
         *
         * @param buttonElement
         */confirmPinVisibilityButtonClicked:function confirmPinVisibilityButtonClicked(buttonElement){if($(buttonElement).attr("data-ally-visible")==="no"){// pin is not visible, make it visible
...
return Promise.all([responseArray[0],ons.notification.alert({title:'<ons-icon icon="md-check-circle" size="32px" '+'style="color: green;"></ons-icon> Wallet Funded',messageHTML:'<span>AMOUNT CHARGED: '+kendo.toString(kendo.parseFloat(responseArray[0].data.amount),'n2')+'</span>',cancelable:false})]);}).then(function(){hockeyapp.trackEvent(function(){},function(){},"WALLET FUNDED");// track wallet funding
...
utopiasoftware.ally.controller.walletTransferPageViewModel.transferModeDropdown=new ej.dropdowns.DropDownList({placeholder:"Select Mode",floatLabelType:'Auto',change:function change(){// change event handler
...
if(utopiasoftware.ally.controller.walletTransferPageViewModel.cardDropDownList){// repopulate the financial cards dropdown list
...
$('#menu-tabbar').get(0).setActiveTab(2);}},/**
         ...
         */pickContactButtonClicked:function(){var _ref=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee(){var permissionStatuses;return regeneratorRuntime.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:permissionStatuses=null;// holds the statuses of the runtime permissions requested
// request runtime permissions to enable access to phone contacts
_context.next=3;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.READ_CONTACTS,cordova.plugins.diagnostic.permission.GET_ACCOUNTS]);});case 3:permissionStatuses=_context.sent;// display the list of contacts from the user's phone address book
new Promise(function(resolve,reject){// check if the user has given permission to write pdf file to device
if(!permissionStatuses||permissionStatuses[cordova.plugins.diagnostic.permission.READ_CONTACTS]!==cordova.plugins.diagnostic.permissionStatus.GRANTED||permissionStatuses[cordova.plugins.diagnostic.permission.GET_ACCOUNTS]!==cordova.plugins.diagnostic.permissionStatus.GRANTED){throw"error - no runtime permissions";}window.plugins.contactNumberPicker.pick(resolve,reject);}).then(function(contact){// retrieve picked contact
...
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});});case 5:case'end':return _context.stop();}}},_callee,this);}));function pickContactButtonClicked(){return _ref.apply(this,arguments);}return pickContactButtonClicked;}(),/**
...
$('#app-main-navigator').get(0).pushPage('add-card-wallet-transfer-page.html',{data:formData,animation:'lift-md'});},/**
         ...
         * @param buttonElem
         * @returns {Promise}
         */walletTransferSmsConfirmButtonClicked:function(){var _ref2=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee2(buttonElem){var walletTransferDetails,permissionStatuses;return regeneratorRuntime.wrap(function _callee2$(_context2){while(1){switch(_context2.prev=_context2.next){case 0:hockeyapp.trackEvent(function(){},function(){},"FUND TRANSFERRED");// track fund transfer
// get the details of the wall transfer for which an sms confirmation is being sent
walletTransferDetails=JSON.parse($(buttonElem).attr('data-wallet-transfer'));permissionStatuses=null;// holds the statuses of the runtime permissions requested
// request runtime permissions to send SMS on the device
_context2.next=5;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.SEND_SMS]);});case 5:permissionStatuses=_context2.sent;// use a promise to send the sms confirmation message to the recipient
new Promise(function(resolve,reject){// check if the user has given permission to send sms on device
if(!permissionStatuses||permissionStatuses[cordova.plugins.diagnostic.permission.SEND_SMS]!==cordova.plugins.diagnostic.permissionStatus.GRANTED){throw"error - no runtime permissions";}// send sms
...
$('#wallet-transfer-page #wallet-transfer-form').get(0).reset();// reset the form validator object on the page
utopiasoftware.ally.controller.walletTransferPageViewModel.formValidator.reset();return _context2.abrupt('return',Promise.all([ons.notification.toast("Wallet Transfer Successful!",{timeout:4000})]));case 11:case'end':return _context2.stop();}}},_callee2,this);}));function walletTransferSmsConfirmButtonClicked(_x2){return _ref2.apply(this,arguments);}return walletTransferSmsConfirmButtonClicked;}()},/**
...
$('.page-preloader',$thisPage).css('display',"block");// hide the form
$('#disburse-wallet-form',$thisPage).css('display',"none");// initialise the bank DropDown widget
...
utopiasoftware.ally.controller.disburseWalletPageViewModel.banksDropDownList.appendTo('#disburse-wallet-bank');// initialise the account number combo box widget
...
dataSource:[],fields:{text:'displayText',value:'ACCOUNTNUMBER'},placeholder:"Account Name or Number (NUBAN)",floatLabelType:"Auto",popupHeight:"300px",allowCustom:true});utopiasoftware.ally.controller.disburseWalletPageViewModel.accountNumberComboBox.appendTo('#disburse-wallet-account-number');// initialise form tooltips
...
utopiasoftware.ally.controller.disburseWalletPageViewModel.banksDropDownList.dataSource=promiseArray[1];utopiasoftware.ally.controller.disburseWalletPageViewModel.banksDropDownList.dataBind();// populate data for the account combo box
utopiasoftware.ally.controller.disburseWalletPageViewModel.accountNumberComboBox.dataSource=promiseArray[0];utopiasoftware.ally.controller.disburseWalletPageViewModel.accountNumberComboBox.dataBind();// add the change event listener to listen for changes in the bank account number combo box
...
dataObject.DDATE=kendo.parseDate(dataObject.DDATE,"yyyy-MM-dd HH:mm:ss");return dataObject;// return the modified object
         ...
         */scanButtonClicked:function(){var _ref3=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee4(){var permissionStatuses;return regeneratorRuntime.wrap(function _callee4$(_context4){while(1){switch(_context4.prev=_context4.next){case 0:permissionStatuses=null;// holds the permission statuses for the permissions requested
_context4.next=3;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.CAMERA]);});case 3:permissionStatuses=_context4.sent;// remove any tooltip being displayed on all forms on the page
...
QRScanner.pausePreview(function(){var _ref4=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee3(status){var qrCodeSegmentsArray,serverResponse;return regeneratorRuntime.wrap(function _callee3$(_context3){while(1){switch(_context3.prev=_context3.next){case 0:// get each content of the QR Code
qrCodeSegmentsArray=(qrCode+"").trim().split("|");// update the contents of the payment form with the qrCodeSegmentsArray
$('#payments-ally-direct-page #payments-ally-direct-merchant-name').val(qrCodeSegmentsArray[0]);$('#payments-ally-direct-page #payments-ally-direct-merchant-code').val(qrCodeSegmentsArray[1]);$('#payments-ally-direct-page #payments-ally-direct-merchant-phone').val(qrCodeSegmentsArray[2]);// remove page transparency
$('html, body').removeClass('ally-transparent');$('#payments-page').removeClass('transparent');$('#payments-ally-scan-page').removeClass('transparent');// hide the payment-ally-scan-modal & display the hour loader modal to let user know app is busy
_context3.next=9;return Promise.all([$('#payments-ally-scan-modal').get(0).hide(),$('#hour-glass-loader-modal').get(0).show()]);case 9:// reset the form validator object on the page
utopiasoftware.ally.controller.paymentsAllyScanPageViewModel.formValidator.reset();// reset the form
$('#payments-ally-scan-page #payments-ally-scan-form').get(0).reset();_context3.prev=11;_context3.next=14;return Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/get-discount-status.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:{merchantcode:qrCodeSegmentsArray[1]}}));case 14:serverResponse=_context3.sent;// convert the server response to valid json
serverResponse=JSON.parse((serverResponse+"").trim());// check if the merchant to be paid has an active discount
if(serverResponse.status=="success"&&serverResponse.discount>0){// the merchant has an active discount
// display the discount banner for merchant on the payments-ally-direct-page
$('#payments-ally-direct-page .payments-ally-direct-discount-banner').css("display","block");$('#payments-ally-direct-page .payments-ally-direct-discount-banner .payments-ally-direct-discount-value').html(serverResponse.discount);}else{// the merchant does not have any active discount
// hide the discount banner for merchant on the payments-ally-direct-page
$('#payments-ally-direct-page .payments-ally-direct-discount-banner').css("display","none");}_context3.next=21;break;case 19:_context3.prev=19;_context3.t0=_context3['catch'](11);case 21:_context3.prev=21;// hide the hour loader & proceed to payment
Promise.all([$('#hour-glass-loader-modal').get(0).hide(),$('#payments-page #payments-tabbar').get(0).setActiveTab(1)]);return _context3.finish(21);case 24:case'end':return _context3.stop();}}},_callee3,this,[[11,19,21,24]]);}));return function(_x3){return _ref4.apply(this,arguments);};}());});});});case 13:case'end':return _context4.stop();}}},_callee4,this);}));function scanButtonClicked(){return _ref3.apply(this,arguments);}return scanButtonClicked;}(),/**
...
// remove the required attribute for the card number dropdown
$('#payments-ally-direct-card-number').removeAttr("required");// so animatedly hide the card selection
anime({targets:'#payments-ally-direct-card-section',height:"0em",duration:450});break;case"card payment":// user selected card payment
// add the required attribute for the card dropdown
$('#payments-ally-direct-card-number').attr("required",true);// so animatedly show the card selection
anime({targets:'#payments-ally-direct-card-section',height:"3.5em",duration:450,complete:function complete(){$('#payments-ally-direct-card-section').css("height","auto");}});break;}}});utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.paymentModeDropdown.appendTo('#payments-ally-direct-payment-mode');// initialise the card DropDown widget
...
width:'100%',allowTextWrap:true,allowPdfExport:true,toolbar:['pdfexport'],columns:[{field:'paymentReceiptDesc',headerText:' ',width:"50%",clipMode:'ellipsiswithtooltip',customAttributes:{class:'payment-receipt-item-desc'}},{field:'paymentReceiptValue',headerText:' ',width:"50%",clipMode:'ellipsiswithtooltip'}],dataSource:[],pdfExportComplete:function(){var _ref5=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee5(pdfExportCompleteArgs){var fileObj,pdfExportBlob,permissionStatuses;return regeneratorRuntime.wrap(function _callee5$(_context5){while(1){switch(_context5.prev=_context5.next){case 0:console.log("ARGUMENTS",pdfExportCompleteArgs);fileObj=null;// variable holds the file object to be created
pdfExportBlob=null;// holds the blob for the pdf content being exported
...
_context5.next=6;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE,cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]);});case 6:permissionStatuses=_context5.sent;// get the blob data when the export process is completed
...
window.plugins.toast.hide();}});}).catch(function(err){console.log("EXPORT FAILED",err);});case 8:case'end':return _context5.stop();}}},_callee5,this);}));function pdfExportComplete(_x4){return _ref5.apply(this,arguments);}return pdfExportComplete;}()});//append the newly created grid
...
$(fieldInstance.$element).removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).removeAttr("data-hint");$(fieldInstance.$element).removeAttr("title");});// listen for the form validation success
utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.formValidator.on('form:success',utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.formValidated);// create the form data to be sent
var formData={phone:utopiasoftware.ally.model.appUserDetails.phone};// start a promise chain to setup the page
Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/get-my-cards.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:formData// data to submit to server
})).then(function(serverResponse){serverResponse+="";serverResponse=JSON.parse(serverResponse.trim());// get the response object
...
utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.cardDropDownList.dataBind();}).then(function(){// hide the page preloader
$('.page-preloader',$thisPage).css('display',"none");// display the form
$('#payments-ally-direct-form',$thisPage).css('display',"block");// hide the loader
$('#loader-modal').get(0).hide();}).catch(function(err){// hide the page preloader
$('.page-preloader',$thisPage).css('display',"none");// display the form
$('#payments-ally-direct-form',$thisPage).css('display',"block");// hide the loader
$('#loader-modal').get(0).hide();});};},/**
...
$('#payments-ally-direct-page [data-hint]').removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$('#payments-ally-direct-page [title]').removeAttr("title");$('#payments-ally-direct-page [data-hint]').removeAttr("data-hint");$('#payments-ally-direct-page #payments-ally-direct-add-card').css("transform","scale(1)");// destroy the form validator objects on the page
...
var formData={phone_sender:utopiasoftware.ally.model.appUserDetails.phone,phone_receiver:$('#payments-ally-direct-page #payments-ally-direct-merchant-phone').val(),merchantcode:$('#payments-ally-direct-page #payments-ally-direct-merchant-code').val(),amount:kendo.parseFloat($('#payments-ally-direct-page #payments-ally-direct-amount').val()),merchantname:$('#payments-ally-direct-page #payments-ally-direct-merchant-name').val()};// check the transfer mode for the merchant payment
if(utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.paymentModeDropdown.value=="wallet payment"){// add the specified payment mode to the form data
formData.payment_mode="wallet payment";}// check the transfer mode for the merchant payment
if(utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.paymentModeDropdown.value=="card payment"){// add the specified card mode and card number to the form data
formData.payment_mode="card payment";formData.cardno=utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.cardDropDownList.value;}ons.notification.prompt({title:"ALLY Secure PIN Confirmation",id:'pin-security-check2',messageHTML:'<div><ons-icon icon="ion-lock-combination" size="24px"\n                    style="color: #30a401; float: left; width: 26px;"></ons-icon>\n                    <span style="float: right; width: calc(100% - 26px);">\n                    Confirm merchant payment by providing your ALLY Secure PIN</span></div>',cancelable:false,placeholder:"Secure PIN",inputType:"number",defaultValue:"",autofocus:false,submitOnEnter:true}).then(function(userInput){// get the user input
...
ons.notification.alert({title:'<ons-icon icon="md-close-circle-o" size="32px" '+'style="color: red;"></ons-icon> ALLY Payment Error',messageHTML:'<span>'+err+'</span>',cancelable:false});});},/**
         * method is triggered when the "Add Card" button is clicked
         */addCardButtonClicked:function addCardButtonClicked(){// generate the formData that will be passed over to the "Add Card (Merchant Payment) page
// create the form data to be submitted
var formData={phone_sender:utopiasoftware.ally.model.appUserDetails.phone,phone_receiver:$('#payments-ally-direct-page #payments-ally-direct-merchant-phone').val(),merchantcode:$('#payments-ally-direct-page #payments-ally-direct-merchant-code').val(),amount:kendo.parseFloat($('#payments-ally-direct-page #payments-ally-direct-amount').val()),merchantname:$('#payments-ally-direct-page #payments-ally-direct-merchant-name').val()};// check the payment mode for the merchant payment
if(utopiasoftware.ally.controller.paymentsAllyDirectPageViewModel.paymentModeDropdown.value=="wallet transfer"){// add the specified payment mode to the form data
...
}).then(function(){// send push notification that account was updated
var pushNotification={// create the push notification object
"app_id":"d5d2bdba-eec0-46b1-836e-c5b8e318e928","filters":[{"field":"tag","key":"phone","relation":"=","value":utopiasoftware.ally.model.appUserDetails.phone}],"contents":{"en":"Your ALLY secure PIN has just been changed"},"headings":{"en":"ALLY PIN Changed"},"android_channel_id":"9fd6b6ae-9937-476f-8ae9-7e9958763532","android_visibility":1,"priority":10};Promise.resolve($.ajax({url:"https://onesignal.com/api/v1/notifications",type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic MmQ3ODcwZGUtYmIyYS00NzY5LWIwZWQtMTk5ZGRjNzU2M2Q3");},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify(pushNotification)}));// inform user that PIN has been changed
ons.notification.toast("PIN changed successfully",{timeout:3000});}).catch(function(err){if(typeof err!=="string"){// if err is NOT a String
...
ons.notification.toast("PDF Exported to Root Folder!",{timeout:4000});}).catch(function(err){console.log("EXPORT FAILED",err);});case 7:case'end':return _context6.stop();}}},_callee6,this);}));function pdfExportComplete(_x5){return _ref6.apply(this,arguments);}return pdfExportComplete;}()});// remove the loader content
...
console.log("DATASOURCE ",utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid.dataSource);utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid.pdfExport({pageOrientation:'landscape',includeHiddenColumn:true,pageSize:'a4',header:{fromTop:0,height:130,contents:[{type:'text',value:"ALLY Transaction History",position:{x:30,y:50},style:{textBrushColor:'#000000',fontSize:14,hAlign:'center',bold:true}}]}},null,null,true);}};});return;// exit method
}//THERE IS AN INTERNET CONNECTION
// request for the user wallet transfer-in data for the provided time period
...
// format the chart data array so it can be properly used
return gridDataMapping(dataArray);}).then(function(dataArray){var _ref8;console.log(dataArray);utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid=new ej.grids.Grid((_ref8={// Width for grid
width:'100%',allowTextWrap:true,showColumnChooser:true},_defineProperty(_ref8,'allowTextWrap',true),_defineProperty(_ref8,'showColumnChooser',true),_defineProperty(_ref8,'allowPdfExport',true),_defineProperty(_ref8,'allowExcelExport',true),_defineProperty(_ref8,'toolbar',['search','columnchooser','pdfexport']),_defineProperty(_ref8,'columns',[{field:'SENDER',headerText:'Sender',width:"25%",clipMode:'ellipsiswithtooltip'},{field:'RECEIVER',headerText:'Recipient',width:"25%",clipMode:'ellipsiswithtooltip'},{field:'AMOUNT',headerText:'Amount',width:"25%",textAlign:'right',clipMode:'ellipsiswithtooltip'},{field:'DDATE',headerText:'Date',width:"25%",clipMode:'ellipsiswithtooltip'},{field:'TRANSFERTYPE',headerText:'Type',width:"25%",clipMode:'ellipsiswithtooltip',visible:false},{field:'TRANSACTIONREF',headerText:'Ref',width:"25%",clipMode:'ellipsiswithtooltip',visible:false}]),_defineProperty(_ref8,'dataSource',dataArray),_defineProperty(_ref8,'pdfExportComplete',function(){var _ref7=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee7(pdfExportCompleteArgs){var fileObj,permissionStatuses;return regeneratorRuntime.wrap(function _callee7$(_context7){while(1){switch(_context7.prev=_context7.next){case 0:console.log("ARGUMENTS",pdfExportCompleteArgs);fileObj=null;// variable holds the file object to be created
permissionStatuses=null;// holds the statuses of the runtime permissions requested
// request runtime permissions to write pdf file to device
_context7.next=5;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE,cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]);});case 5:permissionStatuses=_context7.sent;// get the blob data when the export process is completed
pdfExportCompleteArgs.promise.then(function(pdfData){// get the pdf structure if the content being exported
pdfExportBlob=pdfData.blobData;// get the blob for the exported pdf
console.log("EXPORTED",pdfData);// check if the user has given permission to write pdf file to device
if(!permissionStatuses||permissionStatuses[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE]!==cordova.plugins.diagnostic.permissionStatus.GRANTED||permissionStatuses[cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]!==cordova.plugins.diagnostic.permissionStatus.GRANTED){throw"error - no permissions";}return new Promise(function(resolve,reject){// return the directory where to store the document/image
...
ons.notification.toast("PDF Exported to Root Folder!",{timeout:4000});}).catch(function(err){console.log("EXPORT FAILED",err);});case 7:case'end':return _context7.stop();}}},_callee7,this);}));function pdfExportComplete(_x6){return _ref7.apply(this,arguments);}return pdfExportComplete;}()),_ref8));// remove the loader content
$('#transaction-history-page #transaction-history-transaction-grid').html("");//append the newly created grid
utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid.appendTo('#transaction-history-transaction-grid');// append the listener for the toolbar 'Export PDF' button click
utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid.toolbarClick=function(args){console.log("ID ",args.item.id);if(args.item.id==='transaction-history-transaction-grid_pdfexport'){// the toolbar button being clicked is the 'PDF Export'
console.log("DATASOURCE ",utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid.dataSource);utopiasoftware.ally.controller.transactionHistoryPageViewModel.transactionHistoryGrid.pdfExport({includeHiddenColumn:true,pageSize:'a4',pageOrientation:'landscape',header:{fromTop:0,height:130,contents:[{type:'text',value:"ALLY Transaction History",position:{x:30,y:50},style:{textBrushColor:'#000000',fontSize:14,hAlign:'center',bold:true}}]}},null,null,true);}};});/**
...
$(fieldInstance.$element).removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).removeAttr("data-hint");$(fieldInstance.$element).removeAttr("title");});// listen for the form validation success
utopiasoftware.ally.controller.addCardWalletTransferPageViewModel.formValidator.on('form:success',utopiasoftware.ally.controller.addCardWalletTransferPageViewModel.formValidated);// populate the form using the data retrieved from the previous page
var prePopulatedData=$('#app-main-navigator').get(0).topPage.data;$('#add-card-wallet-transfer-charge-amount',$thisPage).val(kendo.toString(prePopulatedData.amount,"n2"));$('#add-card-wallet-transfer-receiver-phone-number',$thisPage).val(prePopulatedData.phone_receiver);// display the form
$('#add-card-form',$thisPage).css("display","block");// hide the preloader
$('.page-preloader',$thisPage).css("display","none");// hide the loader
$('#loader-modal').get(0).hide();}},/**
         * method is triggered when page is shown
...
var formData={firstName:utopiasoftware.ally.model.appUserDetails.firstname,lastName:utopiasoftware.ally.model.appUserDetails.lastname,phone_sender:utopiasoftware.ally.model.appUserDetails.phone,email:utopiasoftware.ally.model.appUserDetails.email?utopiasoftware.ally.model.appUserDetails.email:"",cardno:utopiasoftware.ally.controller.addCardWalletTransferPageViewModel.cardMaskedTextInput.value,cvv:$('#add-card-wallet-transfer-page #add-card-wallet-transfer-cvv').val(),expirymonth:utopiasoftware.ally.controller.addCardWalletTransferPageViewModel.cardMonthDropDownList.value,expiryyear:utopiasoftware.ally.controller.addCardWalletTransferPageViewModel.cardYearDropDownList.value,pin:$('#add-card-wallet-transfer-page #add-card-wallet-transfer-pin').val(),amount:kendo.parseFloat($('#add-card-wallet-transfer-page #add-card-wallet-transfer-charge-amount').val()),phone_receiver:$('#add-card-wallet-transfer-page #add-card-wallet-transfer-receiver-phone-number').val()};if(formData.phone_receiver.startsWith("0")){// the phone number starts with 0, replace it with international dialing code
...
$('#hour-glass-loader-modal .modal-message').html("Completing Wallet Transfer...");// forward the form data & show loader
Promise.all([formData,Promise.resolve($('#hour-glass-loader-modal').get(0).show())]).then(function(dataArray){// submit the form data
return Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/transfer-wallet-to-wallet-via-card.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:dataArray[0]// data to submit to server
}));}).then(function(serverResponse){serverResponse+="";serverResponse=JSON.parse(serverResponse.trim());// get the response object
// check if any error occurred
if(serverResponse.status!="success"){// an error occured
...
$('#hour-glass-loader-modal .modal-message').html("Authorizing Wallet Transfer...");return Promise.all([].concat(_toConsumableArray(responseArray),[$('#hour-glass-loader-modal').get(0).show()]));}).then(function(responseArray){// create the data object to be sent
var submitData=responseArray[0];// send back previously received response
submitData.otp=responseArray[1];submitData.phone_sender=utopiasoftware.ally.model.appUserDetails.phone;submitData.phone_receiver=formData.phone_receiver;submitData.savecard=$('#add-card-wallet-transfer-page #add-card-wallet-transfer-save-card-details').get(0).checked;// submit the form data
return Promise.all([Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/transfer-wallet-to-wallet-via-card-confirm-otp.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:submitData// data to submit to server
})),responseArray[0]]);}).then(function(serverResponseArray){// serverResponse +=  "";
...
}).then(function(dataArray){// check if the recipient of the wallet transfer is a registered user
if(dataArray[1].isregistereduser!="yes"){// append the json details for the wallet-transfer to the wallet-transfer-sms-confirm-modal confirmation button
$($('#wallet-transfer-sms-confirm-modal #wallet-transfer-sms-confirm-button').get(0)).attr("data-wallet-transfer",JSON.stringify({receiver:formData.phone_receiver,amount:formData.amount}));// show the wallet-transfer-sms-confirm-modal to the user
return $('#wallet-transfer-sms-confirm-modal').get(0).show();}else{// recipient is registered
return"registered recipient";}}).then(function(){hockeyapp.trackEvent(function(){},function(){},"FUND TRANSFERRED");// track fund transfer
// reset the form for the add card wallet transfer page
$('#add-card-wallet-transfer-page #add-card-wallet-transfer-form').get(0).reset();// reset the form validator object on the page
utopiasoftware.ally.controller.addCardWalletTransferPageViewModel.formValidator.reset();// send push notification to the recipient of the transfer
var pushNotification={// create the push notification object
"app_id":"d5d2bdba-eec0-46b1-836e-c5b8e318e928","filters":[{"field":"tag","key":"phone","relation":"=","value":formData.phone_receiver}],"contents":{"en":"You received funds into your ALLY WALLET from "+utopiasoftware.ally.model.appUserDetails.firstname+" "+utopiasoftware.ally.model.appUserDetails.lastname},"headings":{"en":"Funds Received"},"android_channel_id":"66dfeddf-12d7-4194-b3d9-38325042d258","android_visibility":0,"priority":5};Promise.resolve($.ajax({url:"https://onesignal.com/api/v1/notifications",type:"post",contentType:"application/json",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("Authorization","Basic MmQ3ODcwZGUtYmIyYS00NzY5LWIwZWQtMTk5ZGRjNzU2M2Q3");},dataType:"json",timeout:240000,// wait for 4 minutes before timeout of request
processData:false,data:JSON.stringify(pushNotification)}));return Promise.all([ons.notification.toast("Recipient Wallet Funded Successfully!",{timeout:4000}),$('#app-main-navigator').get(0).popPage({data:{refresh:true}})]);}).catch(function(err){if(typeof err!=="string"){// if err is NOT a String
err="Sorry. Recipient ALLY wallet could not be funded. Please retry";}$('#hour-glass-loader-modal').get(0).hide();// hide loader
ons.notification.alert({title:'<ons-icon icon="md-close-circle-o" size="32px" '+'style="color: red;"></ons-icon> Wallet Funding Failed',messageHTML:'<span>'+err+'</span>',cancelable:false});});},/**
         * method is triggered when back button or device back button is clicked
         */backButtonClicked:function backButtonClicked(){// check if the side menu is open
if($('ons-splitter').get(0).right.isOpen){// side menu open, so close it
...
$('#app-main-navigator').get(0).popPage();},/**
         * method is triggered when the 'Pick Contact" button is clicked
         */pickContactButtonClicked:function(){var _ref9=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee8(){var permissionStatuses;return regeneratorRuntime.wrap(function _callee8$(_context8){while(1){switch(_context8.prev=_context8.next){case 0:permissionStatuses=null;// holds the statuses of the runtime permissions requested
// request runtime permissions to enable access to phone contacts
_context8.next=3;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.READ_CONTACTS,cordova.plugins.diagnostic.permission.GET_ACCOUNTS]);});case 3:permissionStatuses=_context8.sent;// display the list of contacts from the user's phone address book
new Promise(function(resolve,reject){// check if the user has given permission to write pdf file to device
if(!permissionStatuses||permissionStatuses[cordova.plugins.diagnostic.permission.READ_CONTACTS]!==cordova.plugins.diagnostic.permissionStatus.GRANTED||permissionStatuses[cordova.plugins.diagnostic.permission.GET_ACCOUNTS]!==cordova.plugins.diagnostic.permissionStatus.GRANTED){throw"error - no runtime permissions";}window.plugins.contactNumberPicker.pick(resolve,reject);}).then(function(contact){// retrieve picked contact
// get the selected contact phone number
var contactPhoneNumber=contact.phoneNumber;//format the retrieved phone number to acceptable app standards
...
window.plugins.toast.showWithOptions({message:"phone contacts could not be accessed right now",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});});case 5:case'end':return _context8.stop();}}},_callee8,this);}));function pickContactButtonClicked(){return _ref9.apply(this,arguments);}return pickContactButtonClicked;}()},/**
...
if(!ons.isReady()||utopiasoftware.ally.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
$('#app-main-navigator').get(0).topPage.onDeviceBackButton=utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.backButtonClicked;// initialise the card masked input widget
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMaskedTextInput=new ej.inputs.MaskedTextBox({mask:'0000 0000 0000 0000 99999',placeholder:"Card Number",promptChar:'*',floatLabelType:"Always"});// render initialized card masked input widget
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMaskedTextInput.appendTo('#add-card-merchant-payment-card-number');// initialise the card month DropDown widget
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMonthDropDownList=new ej.dropdowns.DropDownList({dataSource:[{value:"01",displayText:"01"},{value:"02",displayText:"02"},{value:"03",displayText:"03"},{value:"04",displayText:"04"},{value:"05",displayText:"05"},{value:"06",displayText:"06"},{value:"07",displayText:"07"},{value:"08",displayText:"08"},{value:"09",displayText:"09"},{value:"10",displayText:"10"},{value:"11",displayText:"11"},{value:"12",displayText:"12"}],fields:{text:'displayText',value:'value'},placeholder:"Expiry Month",floatLabelType:"Auto"});// render the initialized card month dropdown list
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMonthDropDownList.appendTo('#add-card-merchant-payment-expiry-month');// initialise the array to hold the valid card expiry years
var cardYearsArray=[];var yearOption=new Date().getFullYear();// get the current year
// add the current year as an object in the cardYearsArray
cardYearsArray.push({value:(""+yearOption).substring(2,4),displayText:yearOption});// add 3 more years to the cardYearsArray for the Card Expiry Year
for(var index=0;index<3;index++){// increase the yearOption by 1
yearOption+=1;// add the additional year as an object in the cardYearsArray
cardYearsArray.push({value:(""+yearOption).substring(2,4),displayText:yearOption});}// initialise the card expiry year DropDown widget
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardYearDropDownList=new ej.dropdowns.DropDownList({dataSource:cardYearsArray,fields:{text:'displayText',value:'value'},placeholder:"Expiry Year",floatLabelType:"Auto"});// render the initialized card expiry year dropdown list
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardYearDropDownList.appendTo('#add-card-merchant-payment-expiry-year');// initialise form tooltips
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formTooltip=new ej.popups.Tooltip({target:'.ally-input-tooltip',position:'top center',cssClass:'ally-input-tooltip',opensOn:'focus'});// render the initialized form tooltip
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formTooltip.appendTo('#add-card-merchant-payment-form');// initialise the card number field validator
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardNumberFieldValidator=$('#add-card-merchant-payment-card-number').parsley({value:function value(parsley){// return the unmasked input from the card number field
return utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMaskedTextInput.value;}});// initialise the amount field
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.amountFieldValidator=$('#add-card-merchant-payment-charge-amount').parsley({value:function value(parsley){// convert the amount back to a plain text without the thousand separator
var parsedNumber=kendo.parseFloat($('#add-card-merchant-payment-charge-amount',$thisPage).val());return parsedNumber?parsedNumber:$('#add-card-merchant-payment-charge-amount',$thisPage).val();}});// initialise the form validation
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator=$('#add-card-merchant-payment-form').parsley();// attach listener for the pay button on the page
$('#add-card-merchant-payment-pay-button').get(0).onclick=function(){// run the validation method for the form
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator.whenValidate();};// listen for the form field validation failure event
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator.on('field:error',function(fieldInstance){// get the element that triggered the field validation error and use it to display tooltip
// display tooltip
$(fieldInstance.$element).addClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).attr("data-hint",fieldInstance.getErrorsMessages()[0]);$(fieldInstance.$element).attr("title",fieldInstance.getErrorsMessages()[0]);});// listen for the form field validation success event
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).removeAttr("data-hint");$(fieldInstance.$element).removeAttr("title");});// listen for the form validation success
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator.on('form:success',utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidated);// populate the form using the data retrieved from the previous page
var prePopulatedData=$('#app-main-navigator').get(0).topPage.data;$('#add-card-merchant-payment-charge-amount',$thisPage).val(kendo.toString(prePopulatedData.amount,"n2"));$('#add-card-merchant-payment-merchant-phone',$thisPage).val(prePopulatedData.phone_receiver);$('#add-card-merchant-payment-merchant-code',$thisPage).val(prePopulatedData.merchantcode);$('#add-card-merchant-payment-merchant-name',$thisPage).val(prePopulatedData.merchantname);// display the form
$('#add-card-merchant-payment-form',$thisPage).css("display","block");// hide the preloader
$('.page-preloader',$thisPage).css("display","none");// hide the loader
$('#loader-modal').get(0).hide();}},/**
         * method is triggered when page is shown
         *
         ...
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms on the page
$('#add-card-merchant-payment-page [data-hint]').removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$('#add-card-merchant-payment-page [title]').removeAttr("title");$('#add-card-merchant-payment-page [data-hint]').removeAttr("data-hint");// reset the form validator object on the page
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){try{// remove any tooltip being displayed on all forms on the page
$('#add-card-merchant-payment-page [data-hint]').removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$('#add-card-merchant-payment-page [title]').removeAttr("title");$('#add-card-merchant-payment-page [data-hint]').removeAttr("data-hint");// destroy the form validator objects on the page
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardNumberFieldValidator.destroy();utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.amountFieldValidator.destroy();utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formValidator.destroy();// destroy other form components
utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMaskedTextInput.destroy();utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMonthDropDownList.destroy();utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardYearDropDownList.destroy();utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.formTooltip.destroy();}catch(err){}},/**
         * method is triggered when the form is successfully validated
         */formValidated:function formValidated(){// check if Internet Connection is available before proceeding
if(navigator.connection.type===Connection.NONE){// no Internet Connection
// inform the user that they cannot proceed without Internet
...
var formData={firstName:utopiasoftware.ally.model.appUserDetails.firstname,lastName:utopiasoftware.ally.model.appUserDetails.lastname,phone_sender:utopiasoftware.ally.model.appUserDetails.phone,email:utopiasoftware.ally.model.appUserDetails.email?utopiasoftware.ally.model.appUserDetails.email:"",cardno:utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMaskedTextInput.value,cvv:$('#add-card-merchant-payment-page #add-card-merchant-payment-cvv').val(),expirymonth:utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardMonthDropDownList.value,expiryyear:utopiasoftware.ally.controller.addCardMerchantPaymentPageViewModel.cardYearDropDownList.value,pin:$('#add-card-merchant-payment-page #add-card-merchant-payment-pin').val(),amount:kendo.parseFloat($('#add-card-merchant-payment-page #add-card-merchant-payment-charge-amount').val()),phone_receiver:$('#add-card-merchant-payment-page #add-card-merchant-payment-merchant-phone').val(),merchantcode:$('#add-card-merchant-payment-page #add-card-merchant-payment-merchant-code').val(),merchantname:$('#add-card-merchant-payment-page #add-card-merchant-payment-merchant-name').val()};if(formData.phone_receiver.startsWith("0")){// the phone number starts with 0, replace it with international dialing code
formData.phone_receiver=formData.phone_receiver.replace("0","+234");}// display the loader message to indicate that account is being created;
$('#hour-glass-loader-modal .modal-message').html("Completing Merchant Payment...");// forward the form data & show loader
Promise.all([formData,Promise.resolve($('#hour-glass-loader-modal').get(0).show())]).then(function(dataArray){// submit the form data
return Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/pay-merchant-via-card-direct.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:dataArray[0]// data to submit to server
}));}).then(function(serverResponse){serverResponse+="";serverResponse=JSON.parse(serverResponse.trim());// get the response object
console.log("MERCHANT PAY",serverResponse);// check if any error occurred
if(serverResponse.status!="success"){// an error occured
throw serverResponse.message||serverResponse.data.message;// throw the error message attached to this error
}return serverResponse;// forward the server response
}).then(function(serverResponse){// hide the loader
...
$('#hour-glass-loader-modal .modal-message').html("Authorizing Merchant Payment...");return Promise.all([].concat(_toConsumableArray(responseArray),[$('#hour-glass-loader-modal').get(0).show()]));}).then(function(responseArray){// create the data object to be sent
responseArray[0].otp=responseArray[1];responseArray[0].savecard=$('#add-card-merchant-payment-page #add-card-merchant-payment-save-card-details').get(0).checked;// submit the form data
return Promise.all([Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/pay-merchant-via-card-direct-confirm.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:responseArray[0]// data to submit to server
}))]);}).then(function(serverResponseArray){// serverResponse +=  "";
...
min:new Date('2008-01-01'),//sets the max.
max:new Date(),format:'yyyy/MM/dd',change:function change(){var _utopiasoftware$ally$;// listener is triggered when date range values are successfully changed
...
(_utopiasoftware$ally$=utopiasoftware.ally.controller.expenseTrackerPageViewModel).refreshExpenseTrackerDisplays.apply(_utopiasoftware$ally$,argumentArray);}});// render initialized expense DatePicker
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerDateRangerPicker.appendTo('#expense-tracker-custom-date');// initialise the DropDownList
...
utopiasoftware.ally.controller.expenseTrackerPageViewModel.periodDropDownListObject.addEventListener("change",function(){// check if the custom date selector needs to be shown
if(utopiasoftware.ally.controller.expenseTrackerPageViewModel.periodDropDownListObject.value=="custom"){// custom date selector needs to be displayed
$('#expense-tracker-custom-date-container',$thisPage).css("visibility","visible");}else{// hide the custom date selector
$('#expense-tracker-custom-date-container',$thisPage).css("visibility","collapse");// refresh the expense tracker displays
// Provide the currently selected value of the period dropdown list
utopiasoftware.ally.controller.expenseTrackerPageViewModel.refreshExpenseTrackerDisplays(utopiasoftware.ally.controller.expenseTrackerPageViewModel.periodDropDownListObject.value);}});// render initialized DropDownList
utopiasoftware.ally.controller.expenseTrackerPageViewModel.periodDropDownListObject.appendTo('#expense-tracker-period-select');// initialise date range picker tooltio
utopiasoftware.ally.controller.expenseTrackerPageViewModel.dateRangePickerToolTip=new ej.popups.Tooltip({target:'.ally-input-tooltip',position:'bottom right',cssClass:'ally-input-tooltip',opensOn:'focus'});// render date range picker tooltip
utopiasoftware.ally.controller.expenseTrackerPageViewModel.dateRangePickerToolTip.appendTo('#expense-tracker-custom-date-container');// refresh expense tracker grid
utopiasoftware.ally.controller.expenseTrackerPageViewModel.refreshExpenseTrackerDisplays();// hide the loader
$('#loader-modal').get(0).hide();}},/**
         * method is triggered when page is shown
         */pageShow:function pageShow(){// disable the swipeable feature for the app splitter
$('ons-splitter-side').removeAttr("swipeable");},/**
...
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerDateRangerPicker.hide();// check if the menu tabbar exists
if($('#menu-tabbar').get(0)){// the menu tabbar object exists
// move to the previous tab
...
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.showSpinner();}// check if there is internet connection or not
if(navigator.connection.type===Connection.NONE){// there is no internet connection
// inform the user that cached data will be displayed in the absence of internet
window.plugins.toast.showWithOptions({message:"No Internet Connection. Previously cached data will be displayed",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#008000',textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});// load the previously cached data
...
// format the data array so it can be properly used
return gridDataMapping(data[periodType]);}).then(function(dataArray){var _ref11;console.log(dataArray);// check if the Expense Tracker Grid has been created before, if so, destroy it
if(utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid){// grid has previously been created
// hide spinner
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.hideSpinner();// destroy the grid object
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.destroy();}utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid=new ej.grids.Grid((_ref11={// Width for grid
width:'100%',allowTextWrap:true,showColumnChooser:true},_defineProperty(_ref11,'allowTextWrap',true),_defineProperty(_ref11,'showColumnChooser',true),_defineProperty(_ref11,'allowPdfExport',true),_defineProperty(_ref11,'allowExcelExport',true),_defineProperty(_ref11,'allowGrouping',true),_defineProperty(_ref11,'groupSettings',{showDropArea:false,columns:['CATEGORY']}),_defineProperty(_ref11,'toolbar',['search','columnchooser','pdfexport']),_defineProperty(_ref11,'columns',[{field:'CATEGORY',headerText:'Category',width:"25%",clipMode:'ellipsiswithtooltip'},{field:'DESCRIPTION',headerText:'Desc.',width:"25%",clipMode:'ellipsiswithtooltip',allowGrouping:false},{field:'DDATE',headerText:'Date',width:"25%",clipMode:'ellipsiswithtooltip',visible:false,allowGrouping:false},{field:'AMOUNT',headerText:'Amount',width:"25%",textAlign:'right',format:'N2',clipMode:'ellipsiswithtooltip',allowGrouping:false}]),_defineProperty(_ref11,'aggregates',[{columns:[{type:'sum',field:'AMOUNT',format:'N2',groupFooterTemplate:'Sub-total: ${sum}',footerTemplate:'Total: ${sum}'}]}]),_defineProperty(_ref11,'dataSource',dataArray),_defineProperty(_ref11,'pdfExportComplete',function(){var _ref10=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee9(pdfExportCompleteArgs){var fileObj,permissionStatuses;return regeneratorRuntime.wrap(function _callee9$(_context9){while(1){switch(_context9.prev=_context9.next){case 0:console.log("ARGUMENTS",pdfExportCompleteArgs);fileObj=null;// variable holds the file object to be created
permissionStatuses=null;// holds the statuses of the runtime permissions requested
// request runtime permissions to write pdf file to device
_context9.next=5;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE,cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]);});case 5:permissionStatuses=_context9.sent;// get the blob data when the export process is completed
pdfExportCompleteArgs.promise.then(function(pdfData){// get the pdf structure if the content being exported
pdfExportBlob=pdfData.blobData;// get the blob for the exported pdf
console.log("EXPORTED",pdfData);// check if the user has given permission to write pdf file to device
if(!permissionStatuses||permissionStatuses[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE]!==cordova.plugins.diagnostic.permissionStatus.GRANTED||permissionStatuses[cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]!==cordova.plugins.diagnostic.permissionStatus.GRANTED){throw"error - no permissions";}return new Promise(function(resolve,reject){// return the directory where to store the document/image
...
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.hideSpinner();ons.notification.toast("PDF Exported to Root Folder!",{timeout:4000});}).catch(function(err){utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.groupColumn("CATEGORY");utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.hideSpinner();});case 7:case'end':return _context9.stop();}}},_callee9,this);}));function pdfExportComplete(_x9){return _ref10.apply(this,arguments);}return pdfExportComplete;}()),_ref11));//append the newly created grid
...
// format the chart data array so it can be properly used
return gridDataMapping(dataArray[periodType]);}).then(function(dataArray){var _ref13;console.log(dataArray);// check if the Expense Tracker Grid has been created before, if so, destroy it
if(utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid){// grid has previously been created
// hide spinner
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.hideSpinner();// destroy the grid object
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.destroy();}utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid=new ej.grids.Grid((_ref13={// Width for grid
width:'100%',allowTextWrap:true,showColumnChooser:true},_defineProperty(_ref13,'allowTextWrap',true),_defineProperty(_ref13,'showColumnChooser',true),_defineProperty(_ref13,'allowPdfExport',true),_defineProperty(_ref13,'allowExcelExport',true),_defineProperty(_ref13,'allowGrouping',true),_defineProperty(_ref13,'groupSettings',{showDropArea:false,columns:['CATEGORY']}),_defineProperty(_ref13,'toolbar',['search','columnchooser','pdfexport']),_defineProperty(_ref13,'columns',[{field:'CATEGORY',headerText:'Category',width:"25%",clipMode:'ellipsiswithtooltip'},{field:'DESCRIPTION',headerText:'Desc.',width:"25%",clipMode:'ellipsiswithtooltip',allowGrouping:false},{field:'DDATE',headerText:'Date',width:"25%",clipMode:'ellipsiswithtooltip',visible:false,allowGrouping:false},{field:'AMOUNT',headerText:'Amount',width:"25%",textAlign:'right',format:'N2',clipMode:'ellipsiswithtooltip',allowGrouping:false}]),_defineProperty(_ref13,'aggregates',[{columns:[{type:'sum',field:'AMOUNT',format:'N2',groupFooterTemplate:'Sub-total: ${sum}',footerTemplate:'Total: ${sum}'}]}]),_defineProperty(_ref13,'dataSource',dataArray),_defineProperty(_ref13,'pdfExportComplete',function(){var _ref12=_asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee10(pdfExportCompleteArgs){var fileObj,permissionStatuses;return regeneratorRuntime.wrap(function _callee10$(_context10){while(1){switch(_context10.prev=_context10.next){case 0:console.log("ARGUMENTS",pdfExportCompleteArgs);fileObj=null;// variable holds the file object to be created
permissionStatuses=null;// holds the statuses of the runtime permissions requested
// request runtime permissions to write pdf file to device
_context10.next=5;return new Promise(function(resolve,reject){cordova.plugins.diagnostic.requestRuntimePermissions(resolve,reject,[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE,cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]);});case 5:permissionStatuses=_context10.sent;// get the blob data when the export process is completed
pdfExportCompleteArgs.promise.then(function(pdfData){// get the pdf structure if the content being exported
pdfExportBlob=pdfData.blobData;// get the blob for the exported pdf
console.log("EXPORTED",pdfData);// check if the user has given permission to write pdf file to device
if(!permissionStatuses||permissionStatuses[cordova.plugins.diagnostic.permission.WRITE_EXTERNAL_STORAGE]!==cordova.plugins.diagnostic.permissionStatus.GRANTED||permissionStatuses[cordova.plugins.diagnostic.permission.READ_EXTERNAL_STORAGE]!==cordova.plugins.diagnostic.permissionStatus.GRANTED){throw"error - no runtime permissions";}return new Promise(function(resolve,reject){// return the directory where to store the document/image
...
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.groupColumn("CATEGORY");// hide the spinner
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.hideSpinner();ons.notification.toast("PDF Exported to Root Folder!",{timeout:4000});}).catch(function(err){utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.groupColumn("CATEGORY");utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.hideSpinner();});case 7:case'end':return _context10.stop();}}},_callee10,this);}));function pdfExportComplete(_x10){return _ref12.apply(this,arguments);}return pdfExportComplete;}()),_ref13));//append the newly created grid
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.appendTo('#expense-tracker-grid');// append the listener for the toolbar 'Export PDF' button click
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerGrid.toolbarClick=function(args){console.log("ID ",args.item.id);if(args.item.id==='expense-tracker-grid_pdfexport'){// the toolbar button being clicked is the 'PDF Export'
...
// destroy the chart object
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerBarChart.destroy();}// display chart loading indicator
$('#expense-tracker-page #expense-tracker-barchart').html('<div class="title" style="font-size: 0.85em; padding: 0.5em;">\n                        ALLY Expense Tracker -Bar Chart\n                    </div>\n                    <div class="content" style="padding: 0.5em;">\n\n                        <ons-icon icon="md-settings" size="28px" style="color: #30a401;" spin>\n                        </ons-icon>\n                    </div>');// check if there is internet connection or not
if(navigator.connection.type===Connection.NONE){// there is no internet connection
// inform the user that cached data will be displayed in the absence of internet
window.plugins.toast.showWithOptions({message:"No Internet Connection. Previously cached data will be displayed",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#008000',textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});// load the previously cached data
...
// format the chart data array so it can be properly used
return chartDataMapping(chartDataArray[periodType]);}).then(function(chartDataArray){utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerBarChart=new ej.charts.Chart({// Width and height for chart in pixel
width:'100%',height:'100%',margin:{left:0,right:15,top:0,bottom:0},palettes:["#0FD0D0"],title:chartCustomisableSettings.chartTitle,titleStyle:{size:'1em'},tooltip:{enable:true,format:'Category: ${point.x} | Amount: ${point.y}'},// Legend for chart
legendSettings:{visible:false},primaryYAxis:{title:'Amount (N)',valueType:'Double',titleStyle:{size:'1em',textAlignment:'center'},labelFormat:'{value}k',edgeLabelPlacement:'Hide',labelIntersectAction:'Hide'},primaryXAxis:{title:'Expense Category',valueType:'Category',titleStyle:{size:'1em',textAlignment:'center'},edgeLabelPlacement:'Shift',labelRotation:45},series:[{dataSource:chartDataArray,width:5,xName:'CATEGORY',yName:'SUM',name:'Expenses',//Series type as Column
type:'Column'}]});// remove the loader content
$('#expense-tracker-page #expense-tracker-barchart').html("");//append the newly created chart
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerBarChart.appendTo('#expense-tracker-barchart');});return;// exit method
}// create the data to be sent to server
var formData={phone:utopiasoftware.ally.model.appUserDetails.phone,duration:periodType};// check if the periodType == custom . if so append the custom date ranges
if(periodType=='custom'){formData.from_date=kendo.toString(utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerDateRangerPicker.startDate,"yyyy-MM-dd");formData.to_date=kendo.toString(utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerDateRangerPicker.endDate,"yyyy-MM-dd");}//THERE IS AN INTERNET CONNECTION
// request for the user wallet payments-in data for the provided time period
...
chartDataArray[1]=chartDataArray[1];chartDataArray[1][periodType]=chartDataArray[0];return utopiasoftware.ally.expenseTrackerChart.saveExpenseTrackerData(chartDataArray[1]);}).then(function(chartDataArray){// get the chart data array to be used by chart
// format the chart data array so it can be properly used
return chartDataMapping(chartDataArray[periodType]);}).then(function(chartDataArray){utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerBarChart=new ej.charts.Chart({// Width and height for chart in pixel
width:'100%',height:'100%',margin:{left:0,right:15,top:0,bottom:0},palettes:["#0FD0D0"],title:chartCustomisableSettings.chartTitle,titleStyle:{size:'1em'},tooltip:{enable:true,format:'Category: ${point.x} | Amount: ${point.y}'},// Legend for chart
legendSettings:{visible:false},primaryYAxis:{title:'Amount (N)',valueType:'Double',titleStyle:{size:'1em',textAlignment:'center'},labelFormat:'{value}k',edgeLabelPlacement:'Hide',labelIntersectAction:'Hide'},primaryXAxis:{title:'Expense Category',valueType:'Category',titleStyle:{size:'1em',textAlignment:'center'},edgeLabelPlacement:'Shift',labelRotation:45},series:[{dataSource:chartDataArray,width:5,xName:'CATEGORY',yName:'SUM',name:'Expenses',//Series type as Column
type:'Column'}]});// remove the loader content
$('#expense-tracker-page #expense-tracker-barchart').html("");//append the newly created chart
utopiasoftware.ally.controller.expenseTrackerPageViewModel.expenseTrackerBarChart.appendTo('#expense-tracker-barchart');});/**
             * function is used to map the chart data into an appropriate forma that can be displayed inby the chart
             * @param chartDataArray {Array} array containing chart data objects to be mapped
             *
             * @return {Array} an array containing properly formatted objects that can be used by the chart
...
totalSum=chartDataArray.reduce(function(accumulator,currentValue){accumulator+=kendo.parseFloat(currentValue.SUM);return accumulator;},totalSum);return chartDataArray.map(function(dataObject){dataObject.SUM=kendo.parseFloat(dataObject.SUM);dataObject.LABELTEXT=kendo.toString(dataObject.SUM/totalSum,"p");return dataObject;// return the modified object
         ...
         */switchVizTypeButtonClicked:function switchVizTypeButtonClicked(buttonVizTypeState){// handle the different buttonVizTypeState values
switch(buttonVizTypeState){case"chart":// current state is 'chart'. so, display chart now
// move the carousel to the 1st chart item
$('#expense-tracker-page #expense-tracker-carousel').get(0).setActiveIndex(1).then(function(){// display the chart visualisation mover button
...
$('#expense-tracker-page #expense-tracker-add-expense-fab').css("margin-left","43vw");});break;}},/**
         ...
         *
         * @param buttonElem
         */moveVizButtonClicked:function moveVizButtonClicked(buttonElem){var vizMoverPosition=$(buttonElem).attr('data-ally-viz-mover');// get the visualisation mover state of the button
// handle the different vizMoverPosition values
switch(vizMoverPosition){case"up":// current position is 'up'. so, move the carousel to the next higher item
$('#expense-tracker-page #expense-tracker-carousel').get(0).setActiveIndex(0).then(function(){// change the button icon from up to down
$('ons-icon',buttonElem).attr("icon","md-chevron-down");// update the button visualisation mover state attribute of the button
$(buttonElem).attr("data-ally-viz-mover","down");});break;case"down":// current position is 'down'. so, move the carousel to the next lower item
$('#expense-tracker-page #expense-tracker-carousel').get(0).setActiveIndex(1).then(function(){// change the button icon from down to up
$('ons-icon',buttonElem).attr("icon","md-chevron-up");// update the button visualisation mover state attribute of the button
$(buttonElem).attr("data-ally-viz-mover","up");});break;}}},/**
...
utopiasoftware.ally.controller.addExpensePageViewModel.expenseDatePicker=new ej.calendars.DatePicker({placeholder:'Pick Expense Date',floatLabelType:'Auto',readonly:true,// sets the value.
value:new Date(),// sets the min date
//min: new Date(),
//sets the max date
max:new Date(),format:'yyyy-MM-dd',focus:function focus(){// listen for when component has focus
...
utopiasoftware.ally.controller.addExpensePageViewModel.formTooltip=new ej.popups.Tooltip({target:'.ally-input-tooltip',position:'top center',cssClass:'ally-input-tooltip',opensOn:'focus'});// render the initialized form tooltip
utopiasoftware.ally.controller.addExpensePageViewModel.formTooltip.appendTo('#add-expense-form');// initialise the amount field
...
// display tooltip
$(fieldInstance.$element).addClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).attr("data-hint",fieldInstance.getErrorsMessages()[0]);$(fieldInstance.$element).attr("title",fieldInstance.getErrorsMessages()[0]);});// listen for the form field validation success event
utopiasoftware.ally.controller.addExpensePageViewModel.formValidator.on('field:success',function(fieldInstance){// remove tooltip from element
$(fieldInstance.$element).removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$(fieldInstance.$element).removeAttr("data-hint");$(fieldInstance.$element).removeAttr("title");});// listen for the form validation success
utopiasoftware.ally.controller.addExpensePageViewModel.formValidator.on('form:success',utopiasoftware.ally.controller.addExpensePageViewModel.formValidated);// create the form data to be sent
var formData={phone:utopiasoftware.ally.model.appUserDetails.phone};// start a promise chain to setup the page
Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/get-expense-categories.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:formData// data to submit to server
})).then(function(serverResponse){serverResponse+="";serverResponse=JSON.parse(serverResponse.trim());// get the response object
// initialise the category DropDown widget
utopiasoftware.ally.controller.addExpensePageViewModel.expenseCategoryDropdown.dataSource=serverResponse;// bind the new update to the dropdown list
utopiasoftware.ally.controller.addExpensePageViewModel.expenseCategoryDropdown.dataBind();}).then(function(){// hide the page preloader
$('.page-preloader',$thisPage).css('display',"none");// display the form
...
// initialise the card DropDown widget
utopiasoftware.ally.controller.addExpensePageViewModel.expenseCategoryDropdown.dataSource=serverResponse;// bind the new update to the dropdown list
utopiasoftware.ally.controller.addExpensePageViewModel.expenseCategoryDropdown.dataBind();}).catch(function(){});}},/**
         * method is triggered when the page is hidden
         * @param event
         */pageHide:function pageHide(event){try{// remove any tooltip being displayed on all forms on the page
$('#add-expense-page [data-hint]').removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$('#add-expense-page [title]').removeAttr("title");$('#add-expense-page [data-hint]').removeAttr("data-hint");// reset the form validator object on the page
utopiasoftware.ally.controller.addExpensePageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){try{// remove any tooltip being displayed on all forms on the page
$('#add-expense-page [data-hint]').removeClass("hint--always hint--success hint--medium hint--rounded hint--no-animate");$('#add-expense-page [data-hint]').removeAttr("title");$('#add-expense-page [data-hint]').removeAttr("data-hint");// destroy the form validator objects on the page
utopiasoftware.ally.controller.addExpensePageViewModel.amountFieldValidator.destroy();utopiasoftware.ally.controller.addExpensePageViewModel.formValidator.destroy();}catch(err){}},/**
         * method is triggered when the form is successfully validated
         */formValidated:function formValidated(){// check if Internet Connection is available before proceeding
if(navigator.connection.type===Connection.NONE){// no Internet Connection
// inform the user that they cannot proceed without Internet
...
if(!ons.isReady()||utopiasoftware.ally.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
$('#app-main-navigator').get(0).topPage.onDeviceBackButton=utopiasoftware.ally.controller.accountSettingsPageViewModel.backButtonClicked;// add listener for when the push notification setting is changed
$('#account-settings-push-notification',$thisPage).on("change",function(toggleEvent){try{window.plugins.OneSignal.setSubscription(toggleEvent.originalEvent.value);// enable/disable push notification
...
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});});});// add listener for when the sms notification setting is changed
$('#account-settings-sms-notification',$thisPage).on("change",function(toggleEvent){// show page preloader
$('.page-preloader',$thisPage).css("display","block");// update the user's sms notification setting
Promise.resolve($.ajax({url:utopiasoftware.ally.model.ally_base_url+"/mobile/sms-notification-save-status.php",type:"post",contentType:"application/x-www-form-urlencoded",beforeSend:function beforeSend(jqxhr){jqxhr.setRequestHeader("X-ALLY-APP","mobile");},dataType:"text",timeout:240000,// wait for 4 minutes before timeout of request
processData:true,data:{phone:utopiasoftware.ally.model.appUserDetails.phone,status:toggleEvent.originalEvent.value?1:0}})).then(function(serverResponse){serverResponse=serverResponse.trim();if(serverResponse!=="success"){// an error occured
throw"error";}else{// there was no error
// hide page preloader
$('.page-preloader',$thisPage).css("display","none");// inform user that ussd settings have changed
ons.notification.toast((toggleEvent.originalEvent.value?'Enabled':'Disabled')+' SMS notifications',{timeout:3000});}}).catch(function(){// an error occurred
// hide page preloader
$('.page-preloader',$thisPage).css("display","none");// inform the user of the error
window.plugins.toast.showWithOptions({message:"your ALLY SMS notification setting could not be updated right now",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
...
return new Promise(function(resolve,reject){window.plugins.OneSignal.getPermissionSubscriptionState(resolve);// get device push notification status
...
if(pushStatus.subscriptionStatus.subscribed===true&&pushStatus.subscriptionStatus.userSubscriptionSetting===true){// push notification is enabled
...
$('#account-settings-push-notification',$thisPage).get(0).checked=false;}}).then(function(){// get the current user ussd setting from the app server
