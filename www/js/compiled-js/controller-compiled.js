...
utopiasoftware.ally.controller.accountPageViewModel.formValidator.whenValidate();console.log("SAVE BUTTON CLICKED");},/**
...
return utopiasoftware.ally.saveUserAppDetails(userDetails);}).then(function(userDetails){// update the page with the collect data details
// update user data on app
utopiasoftware.ally.model.appUserDetails=userDetails;$('#wallet-balance',$thisPage).html(kendo.toString(kendo.parseFloat(userDetails.balance),"n2"));$('#wallet-owner-name',$thisPage).val(userDetails.firstname+" "+userDetails.lastname);// display the necessary page components
...
return utopiasoftware.ally.saveUserAppDetails(userDetails);}).then(function(userDetails){// update the page with the collected data details
// update user data on app
utopiasoftware.ally.model.appUserDetails=userDetails;$('#wallet-page #wallet-balance').html(kendo.toString(kendo.parseFloat(userDetails.balance),"n2"));$('#wallet-page #wallet-owner-name').val(userDetails.firstname+" "+userDetails.lastname);// display the necessary page components
...
if(!ons.isReady()||utopiasoftware.ally.model.isAppReady===false){setTimeout(loadPageOnAppReady,500);// call this function again after half a second
return;}// listen for the back button event
$('#app-main-navigator').get(0).topPage.onDeviceBackButton=utopiasoftware.ally.controller.fundWalletPageViewModel.backButtonClicked;// initialise the card DropDown widget
...
dataSource:["******2341"],placeholder:"Select Card",popupHeight:"300px"});// render initialized card DropDownList
...
utopiasoftware.ally.controller.fundWalletPageViewModel.formValidator.on('form:success',utopiasoftware.ally.controller.fundWalletPageViewModel.formValidated);// hide the loader
$('#loader-modal').get(0).hide();}},/**
         * method is triggered when page is shown
         *
         * @param event
...
utopiasoftware.ally.controller.fundWalletPageViewModel.formValidator.reset();}catch(err){}},/**
         * method is triggered when the page is destroyed
         * @param event
         */pageDestroy:function pageDestroy(event){console.log("PAGE DESTROYED");try{// remove any tooltip being displayed on all forms on the page
...
utopiasoftware.ally.controller.addCardPageViewModel.cardMonthDropDownList=new ej.dropdowns.DropDownList({dataSource:[{value:"01",displayText:"01"},{value:"02",displayText:"02"},{value:"03",displayText:"03"},{value:"04",displayText:"04"},{value:"05",displayText:"05"},{value:"06",displayText:"06"},{value:"07",displayText:"07"},{value:"08",displayText:"08"},{value:"09",displayText:"09"},{value:"10",displayText:"10"},{value:"11",displayText:"11"},{value:"12",displayText:"12"}],fields:{text:'displayText',id:'value'},placeholder:"Expiry Month",floatLabelType:"Auto"});// render the initialized card month dropdown list
...
utopiasoftware.ally.controller.addCardPageViewModel.cardYearDropDownList=new ej.dropdowns.DropDownList({dataSource:cardYearsArray,fields:{text:'displayText',id:'value'},placeholder:"Expiry Year",floatLabelType:"Auto"});// render the initialized card expiry year dropdown list
...
utopiasoftware.ally.controller.addCardPageViewModel.formTooltip.appendTo('#add-card-form');// initialise the amount field validator
...
return utopiasoftware.ally.controller.addCardPageViewModel.cardMaskedTextInput.value;}});// initialise the form validation
...
$('#add-card-save-button').get(0).onclick=function(){// run the validation method for the form
...
utopiasoftware.ally.controller.addCardPageViewModel.cardNumberFieldValidator.destroy();utopiasoftware.ally.controller.addCardPageViewModel.formValidator.destroy();// destroy other form components
...
if(navigator.connection.type===Connection.NONE){// no Internet Connection
// inform the user that they cannot proceed without Internet
window.plugins.toast.showWithOptions({message:"Financial card cannot be saved without an Internet Connection",duration:4000,position:"top",styling:{opacity:1,backgroundColor:'#ff0000',//red
textColor:'#FFFFFF',textSize:14}},function(toastEvent){if(toastEvent&&toastEvent.event=="touch"){// user tapped the toast, so hide toast immediately
window.plugins.toast.hide();}});return;// exit method immediately
}// create the form data to be submitted
/** var createAcctFormData = {
