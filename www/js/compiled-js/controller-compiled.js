    ...
    accountPageViewModel: {
        ...
        pageInit: function pageInit(event) {
            ...
            function loadPageOnAppReady() {
                ...
                // listen for the form validation success
                utopiasoftware.ally.controller.accountPageViewModel.formValidator.on('form:success', utopiasoftware.ally.controller.accountPageViewModel.accountFormValidated);

                // load the user data
                utopiasoftware.ally.controller.accountPageViewModel.loadUserAccountData().then(function (userDetails) {
                    // save the returned user details in app cache
                    return utopiasoftware.ally.saveUserAppDetails(userDetails);
                }).then(function (userDetails) {
                    // update the page with the collect data details

                    // update user data on app
                    utopiasoftware.ally.model.appUserDetails = userDetails;

                    $('#account-wallet-balance', $thisPage).html(kendo.toString(kendo.parseFloat(userDetails.balance), "n2"));
                    $('#account-last-updated', $thisPage).html(kendo.toString(new Date(userDetails._lastUpdatedDate), "MMM d yyyy, h:mmtt"));
                    ...
                    // an error occurred when trying to load the user data
        ...
        backButtonClicked: function backButtonClicked() {
            ...
            if ($('#menu-tabbar').get(0)) {
                // the menu tabbar object exists
         ...
         */
        editAccountButtonClicked: function editAccountButtonClicked() {
         ...
         */
        refreshAccountButtonClicked: function refreshAccountButtonClicked() {
            ...
            utopiasoftware.ally.controller.accountPageViewModel.loadUserAccountData().then(function (userDetails) {
                ...
                // update the page with the collected data details

                // update user data on app
                utopiasoftware.ally.model.appUserDetails = userDetails;

                $('#account-page #account-wallet-balance').html(kendo.toString(kendo.parseFloat(userDetails.balance), "n2"));
                $('#account-page #account-last-updated').html(kendo.toString(new Date(userDetails._lastUpdatedDate), "MMM d yyyy, h:mmtt"));
         ...
         */
        saveAccountButtonClicked: function saveAccountButtonClicked() {
         ...
         */
        accountFormValidated: function accountFormValidated() {
            // check if Internet Connection is available before proceeding
            if (navigator.connection.type === Connection.NONE) {
                // no Internet Connection
                // inform the user that they cannot proceed without Internet
            ...
            ons.notification.prompt({ title: '<ons-icon icon="ion-lock-combination" size="28px" ' + 'style="color: #30a401;"></ons-icon> Security Check', id: "pin-security-check",
                messageHTML: '<div><span>' + 'Please enter your ALLY Secure PIN to proceed</span></div>',
                cancelable: true, placeholder: "Secure PIN", inputType: "number", defaultValue: "", autofocus: false,
                submitOnEnter: true
            }).then(function (userInput) {
                // collect user secure PIN and submit the rest of the form data
                ...
                // check if any error occurred
                if (userAcctDetails.status == "error") {
                    // an error occured
                ...
                ons.notification.alert({ title: '<ons-icon icon="md-close-circle-o" size="32px" ' + 'style="color: red;"></ons-icon> Account Update Failed',
                    messageHTML: '<span>' + err + '</span>',
                    cancelable: false
                });
            });
         ...
         */
        loadUserAccountData: function loadUserAccountData() {
            return new Promise(function (resolve, reject) {
                ...
                if (navigator.connection.type === Connection.NONE) {
                    ...
                    // there's internet, so make request for new data
    ...
    walletPageViewModel: {
        ...
        pageInit: function pageInit(event) {
            ...
            function loadPageOnAppReady() {
                ...
                // load the user data
                utopiasoftware.ally.controller.walletPageViewModel.loadUserAccountData().then(function (userDetails) {
                    // save the returned user details in app cache
                    return utopiasoftware.ally.saveUserAppDetails(userDetails);
                }).then(function (userDetails) {
                    // update the page with the collect data details
                    ...
                    $('#wallet-balance', $thisPage).html(kendo.toString(kendo.parseFloat(userDetails.balance), "n2"));
        ...
        refreshAccountButtonClicked: function refreshAccountButtonClicked() {
            ...
            utopiasoftware.ally.controller.walletPageViewModel.loadUserAccountData().then(function (userDetails) {
                ...
                $('#wallet-page #wallet-balance').html(kendo.toString(kendo.parseFloat(userDetails.balance), "n2"));
